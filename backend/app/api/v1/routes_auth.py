...
async def _issue_token_pair(user: User, db: AsyncSession) -> Token:
    ...
    db.add(RefreshToken(user_id=user.id, jti=str(uuid.uuid4()), expires_at=expires_at))
    await db.commit()
...
async def refresh_access_token(payload: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    token itself is not rotated here to keep the flow simple; a stricter
    deployment could rotate + revoke the old one on each use."""
    try:
        claims = jwt.decode(
    ...
    return AccessTokenOut(access_token=create_access_token(subject=str(user.id)))
...
async def logout_all_sessions(
    ...
    """Revokes all of the current user's outstanding refresh tokens. Already
    issued access tokens remain valid until they naturally expire (they're
    short-lived by design), but no new access token can be minted without a
